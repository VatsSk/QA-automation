package com.testingautomation.testautomation.config.llmConfig;
...
public class LLMServices {
    ...
    public AIValidationResult analyzeScreenshots(String prompt, List<File> screenshots) {
        System.out.println("Inside analyzeScreenshots()");
        ...
        try {
            ...
            for (File file : screenshots) {
                System.out.println("adding screenshot into content");
...
//        List<Map<String, Object>> content = new ArrayList<>();
//
//        // 🔥 Strong structured prompt (VERY IMPORTANT for Qwen)
////        String finalPrompt = """
...
////                - Focus on UI elements, columns, sorting, and visibility
////
////                Rules:
...
//        Map<String, Object> request = new HashMap<>();
//        request.put("model", "qwen/qwen2.5-vl-72b-instruct"); // 🔥 Qwen model
//        request.put("messages", List.of(message));
//        request.put("temperature", 0);
//
...
//                .header("X-Title", "QA-Automation-Tool")         // required
//                .bodyValue(request)
//                .retrieve()
//                .onStatus(
//                        status -> status.is4xxClientError() || status.is5xxServerError(),
