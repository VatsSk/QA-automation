...
public class RunnerService {
    ...
    public Run executeRun(Run run) {
        String baseUrl=runnerProperties.getBaseUrl();
        String runAuthPath=runnerProperties.getRunAuthPath();
        if (runnerProperties.getBaseUrl() == null || baseUrl.isBlank()) {
            throw new IllegalStateException("Runner base URL is missing");
        }

        if (runAuthPath == null || runAuthPath.isBlank()) {
            throw new IllegalStateException("Runner endpoint is missing");
        }
        String runnerUrl = baseUrl + runAuthPath;
